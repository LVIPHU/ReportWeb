...
$(function () {
    ...
    $('body').on('click', '#genSQL', function () {
        ...
        ajaxRequest('Default.aspx/genSQL', result, function (resp) {
            if (resp.d) {
                if (resp.d.includes("ERR|")) {
                    Swal.fire("Lỗi!", resp.d.replace("ERR|", ""), 'error');
                } else {
                    $("#querySQL").val(resp.d);
    ...
    $('body').on('click', '#removeOneRow', function () {
        if ($('#dataTable tbody tr').length == 8) {
            Swal.fire("Cảnh báo!", "Không xóa dòng được nữa!", 'warning');
            return;
        }
    ...
    $('body').on('click', '#removeOneColumn', function () {
        var noOfColumn = document.getElementById('dataTable').rows[0].cells.length - 1;
        if (noOfColumn == 1) {
            Swal.fire("Cảnh báo!", "Không xóa cột được nữa!", 'warning');
            return;
        }
